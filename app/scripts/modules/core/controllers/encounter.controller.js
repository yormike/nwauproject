    ...
    function ($scope, $timeout, $stateParams, EncounterService, JourneyService, StorageService, ihpaService, $q, $modal, $state) {
      ...
      var updateClasses = function () {
        ...
        if (previousEncounter && previousEncounter.ansnap) {
          ...
          console.log("episodetype:"+previousEncounter);
        }
      ...
      var updateLosChart = function (encounter, journey) {
        if (!$scope.encounter.nwauCalculation || !(encounter.classification === 'Acute Admitted'||encounter.classification === 'MH' || encounter.classification === 'SNAP')) {
          return;
        }
      ...
      var updateIcuState = function () {
        if (($scope.encounter.classification !== 'Acute Admitted')&&($scope.encounter.classification !== 'MH')) {
          return;
        }
      ...
      var doCalculations = function (ignoreEncounterForm) {console.log('doCalculations');
      var facility = _.find($scope.facilities, { Facilities: $scope.journey.facility.Facilities });
      if ($scope.encounter.classification == 'MH' && $scope.journey.nwauVersion > 2017 && facility && (facility.FacilityID === 'L216')) {
        var exceptionModal = $modal.open({
          ...
          resolve: {
            ...
            type: function () {
              return 'Please note that Orange Base Hospital is out of scope for Mental Health on NWAU18.';
            }
          }
        });
      ...
      $scope.change = function () {
        console.log("changed");
        if ($scope.journey.facility) {
          var facility = _.find($scope.facilities, { Facilities: $scope.journey.facility.Facilities });
          console.log('FAXILIRY1', facility);
          if ($scope.encounter.classification == 'MH' && $scope.journey.nwauVersion > 2017 && facility && (facility.FacilityID === 'L216')) {
            var exceptionModal = $modal.open({
              templateUrl: 'views/hospital-exception-modal.html',
              controller: 'HospitalExceptoinCtrl',
            ...
            return exceptionModal.result.then(function (str) {
              console.log("result");
              $scope.encounter.classification = '';
              return str;
            })
          }
        }
        if(($scope.encounter.classification=='MH')&&($scope.encounter.psychiatric==null))
        {
          $scope.encounter.psychiatric=true;
        }
        hideAgeLimitDrgs = ($scope.journey.age > 16);
        console.log(hideAgeLimitDrgs);
      ...
      $scope.$watch('encounter.classification', function () {
        ...
        if ($scope.encounter.classification !== 'Acute Admitted' &&
          $scope.encounter.classification !== 'MH' &&
        ...
        if ($scope.encounter.classification === 'MH') {
          $scope.encounter.psychiatricCare = null;
        }
      });
