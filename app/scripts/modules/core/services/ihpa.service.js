    ...
    return {
      getAcute: function (year) {
        return getYearData('Acute', year)
          .then(function (result) {
            //console.log(result.Acute);
            return result.Acute;
          });
      },
      getMH: function (year) {
        //console.log("startGetMH");
        if (year<2017)
          return null;
        return getYearData('MH', year)
          .then(function (result) {
            //console.log("Mh getdata");
            //console.log(year);
            //console.log(result.MH);
            return result.MH;
          });
      },
            ...
            .then(function (result) {
              ...
              return _(suggestions)
                .map(function (suggestion) {
                  ...
                  else if ((/^DRG/).test(suggestion.to_type)) {
                    care = _(mh).find({'DRG': suggestion.to});
                    if (care) {
                      suggestion.classification = 'MH';
                      suggestion.description = suggestion.to + ' - ' + care.Description;
                    }
                  }
