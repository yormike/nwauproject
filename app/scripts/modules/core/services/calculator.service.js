...
angular.module('nwauCalculatorApp').factory('Calculator', ['ihpaService', '$q', '$modal', '$rootScope',
    function(ihpaService, $q, $modal, $rootScope) {
        ...
        var getEDRemoteAdjustment = function(facility, RegionAdj, postcode) {
            if (postcode.Description == 'Remote') {
                return RegionAdj.Remote;
            } else if (postcode.Description == 'Very Remote') {
                return RegionAdj.VeryRemote; 
            } else {
                return 0;
            }
        }
      ...
      return 0;
        ...
        return {
            ...
            EDURG: function(adjustments, urg, year, journey) {
             // if(_debug) console.log('EDURG ', arguments);
                console.log('year', year);
                console.log('nwauVersion', journey.nwauVersion);
                var edurgFormula = function(PW, AInd, AEca, AA) {
                  //if(_debug) console.log('edurgFormula AEca: ', arguments);

                  if (year >= 2016) {
                    if (year > 2017) {
                        return (PW * 100 * ((1 + AInd + AA) * (1 + AEca) * 1000)) / 100000;
                    } else {
                ...
                var edurgCalculate = function(edurg, care, SEP) {
                    ...
                    var A_A = getEDRemoteAdjustment(journey.facility, edurg, adjustments.region);
                    result.nwau = edurgFormula(PW, A_IND, A_ECA, A_A);
                    console.log('AAAA', A_A);
                    ...
                    if (journey.nwauVersion > 2017) { result.adjustments.remoteness = PW * A_A; }
                    result.SEP = SEP;
                    result.PW = PW;
            ...
            EDUDG: function(adjustments, udg, year, journey) {
                console.log('year', year);
                console.log('nwauVersion', journey.nwauVersion);
                var edudgFormula = function(PW, AInd, AEca, AA) {
                  if(AEca) {
                    return PW * (1 + AInd + AA) * (1 + AEca);	// Mark
                  } else {
                    return PW * (1 + AInd + AA);
                  }
                };
                ...
                var edudgCalculate = function(edudg, care, SEP) {
                 ...
                 // if(_debug) console.log('edudgCalculate ', adjustments);
                    ...
                    var result = {}; //nwauCalculation to return
                    result.adjustments = {}; //nwauCalculation includes adjustments, PW and Price
                    var A_A = getEDRemoteAdjustment(journey.facility, edudg, adjustments.region);
                    //setCareAcute(care); //set all the required parameters based on user adjustments and DRG values
                    ...
                    result.nwau = edudgFormula(PW, A_IND, A_ECA, A_A);
                    result.price = result.nwau * SEP;
                    if (journey.nwauVersion > 2017) { result.adjustments.remoteness = PW * A_A; }
                    // adjustments to plot separately
                    result.adjustments.indigenous = PW * A_IND;
